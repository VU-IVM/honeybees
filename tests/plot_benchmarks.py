...
def main(fn):
    ...
    for bits, values in res.items():
        ...
        if bits == max_bits:
            for i in range(halfway):
                ax.annotate(names[i], (x[i], y[i] * offset), rotation='vertical', fontsize=8, fontstyle='italic', ha='center')
        if bits == min_bits:
            for i in range(halfway, len(names)):
                ax.annotate(names[i], (x[i], y[i] * (1/ offset)), rotation='vertical', fontsize=8, fontstyle='italic', ha='center', va='top')
        zorder += 1
    ...
    ax.set_title(f"select {benchmark['extra_info']['n_neighbors']} agents within {benchmark['extra_info']['radius']} m for every 1000th agent", fontsize=10)

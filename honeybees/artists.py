...
class Artists:
    ...
    def draw_global(self):
        return {"type": "shape", "shape": "polygon", "filled": False, "color": "Black", "edge": True}
    ...
    def add_alpha_to_hex_color(self, hex_color, alpha):
        ...
        alpha_hex = hex(decimal_alpha).replace('0x', '')  # all hex values are prefixed with '0x'
    ...
    def generate_discrete_colors(self, n, color, mode, min_alpha=0):
        ...
        if mode == 'hex':
            return [
                self.add_alpha_to_hex_color(color, alpha)
                for alpha in alphas
            ]
        elif mode == 'rgb':
            return [

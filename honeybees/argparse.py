...
parser = argparse.ArgumentParser(description=f"{__name__.title()} is a package for high-speed geographical agent-based modelling in Python")
...
parser.add_argument('--no-browser', dest='browser', action='store_false', help=f"Do not open browser when running the model. This option is, for example, useful when running the model on a server, and you would like to remotely access the model.")
parser.set_defaults(browser=True)
default_port = 8521
parser.add_argument('--port', dest='port', type=int, default=8521, help=f"Port used for display environment (default: {default_port})")